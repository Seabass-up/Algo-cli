...
PRODUCTS = {
    ...
    "grok_build": ProductSpec("grok_build", "Grok Build", "terminal", ("grok",), True),
...
def command_for(
    ...
    if harness == "grok_build":
        ...
        (grok_home / "config.toml").write_text(
            ...
            "\n"
            "[models]\n"
            'default = "wikiskill-local"\n'
            "\n"
            "[model.wikiskill-local]\n"
            f"model = {model_literal}\n"
            'base_url = "http://127.0.0.1:11434/v1"\n'
            'name = "WikiSkill local benchmark"\n'
            'env_key = "GROK_BENCHMARK_API_KEY"\n'
...
def run_process(command: list[str], *, cwd: Path, env: dict[str, str], timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            # A detached descendant can retain the pipes after the process group exits.
            stdout = error.stdout if isinstance(error.stdout, str) else (error.stdout or b"").decode("utf-8", "replace")
            stderr = error.stderr if isinstance(error.stderr, str) else (error.stderr or b"").decode("utf-8", "replace")
        return {
            "return_code": 124,
            "stdout": stdout,
            "stderr": stderr,
...
def run_task_checker(
    ...
    if task_id == "code_repair_small_repo":
        ...
        for relative, wanted in service_expectations.items():
            try:
                service_settings = json.loads((workspace / relative).read_text(encoding="utf-8"))
            except Exception as exc:
...
def event_metrics(harness: str, events: list[dict[str, Any]], state: Path) -> dict[str, Any]:
    ...
    for event in events:
        ...
        if harness == "grok_build" and event.get("method") in {
            "session/update",
            ...
            if isinstance(update, dict):
                ...
                if update_type == "tool_call":
                    tool_ids.add(str(update.get("toolCallId") or update.get("tool_call_id") or len(tool_ids)))
                if update_type == "agent_message_chunk":
                    content = update.get("content")
                    if isinstance(content, str):
                        final_text += content
                    elif isinstance(content, dict):
                        final_text += str(content.get("text") or "")
                    elif isinstance(content, list):
                        final_text += "".join(str(item.get("text") or "") for item in content if isinstance(item, dict))
                update_usage = update.get("usage")
                if isinstance(update_usage, dict):
                    total = update_usage.get("total_tokens", update_usage.get("totalTokens"))
                    if isinstance(total, int):
                        token_total = total
        if harness == "grok_build" and event_type == "text":
            final_text += str(event.get("data") or event.get("text") or "")
