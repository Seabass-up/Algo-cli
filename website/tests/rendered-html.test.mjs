...
test("server-renders the Algo CLI product home", async () => {
  ...
  assert.match(html, /v0\.20\.1 stable/i);
...
test("ships truthful machine-readable release and benchmark contracts", async () => {
  ...
  assert.equal(release.version, "0.20.1");
  assert.equal(release.tag, "v0.20.1");
  assert.equal(release.source_revision, "2fae0bfbf915af0cd204ad81e171d499333efc82");
  assert.equal(release.artifacts.length, 2);
  assert.match(release.artifacts[0].sha256, /^[0-9a-f]{64}$/);
  assert.equal(release.source.release_url, "https://github.com/Seabass-up/Algo-cli/releases/tag/v0.20.1");
  ...
  assert.equal(docs.version, "0.20.1");
