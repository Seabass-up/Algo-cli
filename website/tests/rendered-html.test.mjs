...
test("ships truthful machine-readable release and benchmark contracts", async () => {
  ...
  assert.equal(release.version, "0.20.0");
  assert.equal(release.tag, "v0.20.0");
  assert.equal(release.source_revision, "55a7ec5328892a07129c48e370d175eff8e0eff6");
  assert.equal(release.artifacts.length, 2);
  assert.match(release.artifacts[0].sha256, /^[0-9a-f]{64}$/);
  assert.equal(release.source.release_url, "https://github.com/Seabass-up/Algo-cli/releases/tag/v0.20.0");
  ...
  assert.equal(benchmark.protocol.total_runs, 132);
  ...
  assert.equal(benchmark.protocol.tasks.length, 4);
  assert.equal(benchmark.protocol.repetitions_per_cell, 3);
  assert.equal(benchmark.protocol.model_warmup.success, true);
  assert.equal(benchmark.protocol.model_warmup.included_in_scored_duration, false);
  assert.equal(benchmark.results.find((row) => row.id === "algo-cli").clean_runs, 12);
  assert.equal(benchmark.results.find((row) => row.id === "algo-cli").rank, 1);
  assert.equal(benchmark.top_reliability_group_size, 5);
  assert.match(benchmark.limitations, /do not support a universal superiority/i);
  assert.match(benchmark.limitations, /retained locally/i);
  ...
  assert.equal(docs.version, "0.20.0");
