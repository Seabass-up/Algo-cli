  ...
  return <PageFrame eyebrow="INSTALL / v0.20.0" title="Start local. Add only what you need." intro="Algo CLI v0.20.0 is available from PyPI for Python 3.10 or newer. The algo-cli-runtime distribution installs the algo-cli command.">
    <section className="content-wrap">
      ...
      <div className="install-grid">
        ...
        <article className="install-card"><span>03 / UPGRADE</span><h2>Already installed</h2><p>Update in place from any published 0.19.x. Configuration, credentials, memory, and workspace settings are preserved.</p><CopyInstall command="algo-cli update" /><CopyInstall command="algo-cli --version" /></article>
      ...
      <div className="provider-grid">
        ...
        <article><span>MEMORY</span><h3>Native Continuum, no fallback</h3><CopyInstall command="algo-cli config memory status" /><p className="fine-print">Memory routes only through the separately installed <code>continuum-memory</code> command. Upgrading from a configuration that still names a retired backend? Run <code>algo-cli config memory repair</code>.</p></article>
