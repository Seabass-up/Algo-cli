...
export default function InstallPage() {
  return <PageFrame eyebrow="INSTALL / v0.20.1" title="Start local. Add only what you need." intro="Algo CLI v0.20.1 is available from PyPI for Python 3.10 or newer. The algo-cli-runtime distribution installs the algo-cli command.">
    <section className="content-wrap">
      <div className="notice"><Pill tone="lime">STABLE RELEASE</Pill><p>Version 0.20.1 is published from the immutable v0.20.1 tag through the protected release workflow. Wheel and sdist digests are listed in the <a href="/api/v1/releases/stable.json">release manifest</a>.</p></div>
      <div className="install-grid">
        ...
        <article className="install-card"><span>03 / UPGRADE</span><h2>Already installed</h2><p>Update in place from 0.20.0 or 0.19.2. Configuration, credentials, memory, and workspace settings are preserved. A 0.19.1 install in a pip-free <code>uv pip</code> environment cannot self-update; run <code>uv pip install --upgrade algo-cli-runtime</code> there once.</p><CopyInstall command="algo-cli update" /><CopyInstall command="algo-cli --version" /></article>
        <article className="install-card"><span>04 / SOURCE</span><h2>Reviewed repository</h2><p>Check out the immutable v0.20.1 tag when you want to inspect the exact source. Its release page carries the source archive, SBOM, provenance, and Sigstore bundles.</p><CopyInstall command="git clone https://github.com/Seabass-up/Algo-cli.git && cd Algo-cli && git checkout v0.20.1" /><CopyInstall command="pipx install . && algo-cli doctor" /></article>
      </div>
