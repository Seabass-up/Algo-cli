...
## 2026-09-24: Ctrl+C Used Several Unrelated Cancel Signals, And Approval Cancel Looked Like A Generation Interrupt

**Symptom:** Cancellation was spread across the streaming loop, the parallel batch grace, the `/agent team` `threading.Event` on `cfg._algo_team_cancellation`, the run_shell and write_file checks, and the approval prompt. A detached specialist could still start a tool call after cancel, because dispatch never read the team event. Ctrl+C at an approval prompt reported "Generation interrupted." (one-shot: `interrupted`), the same as a Ctrl+C during model output.

**Confirmed cause:** No single signal existed. Only run_shell and write_file received the team event, only through `run_tool`, and dispatch checked only its per-batch `DispatchCancellation`. The approval path let a bare `KeyboardInterrupt` escape, so every caller mapped it to `keyboard_interrupt`.

**Repair:** `algo_cli/cancellation.py` adds `CancelToken`. It is thread-safe, cancels once and keeps the first reason, and has `raise_if_cancelled()`, `wait()` and `on_cancel()`. Child tokens are cancelled with their parent, never the reverse. `is_set()` keeps the old Event-based tool API working. A context variable holds the ambient token. `agent_loop` and `run_agent_pipeline` bind one token per turn and cancel it when Ctrl+C escapes. Parallel-batch workers inherit it through `copy_context()`, and the batch drain cancels it before the grace wait. `run_agent_team` creates a team token, and each member binds its own child token inside its pool thread. This replaces `_algo_team_cancellation`; the status lock and bounded grace are unchanged. `run_tool` passes the ambient token to run_shell and write_file, and the result wording names the team or the turn. `_DispatchControl` captures the token when dispatch starts, so a cancelled or detached worker never invokes another tool. When Ctrl+C lands at the approval prompt, it cancels the token with `approval_cancelled` and raises `ApprovalCancelled`. If the token is cancelled while the prompt waits, the answer is discarded, and no prompt opens for a turn that is already cancelled. The outcome carries `error_code=approval_cancelled`. The REPL prints "Approval cancelled. The pending action was not approved or run.", and one-shot reports `status_reason=approval_cancelled`. run_shell still starts children in a new session or process group and kills the group with `os.killpg`, or with `taskkill /T /F` on Windows.

**Verification:** `tests/test_cancellation.py` has 22 tests. They cover token semantics: idempotence, reason validation, wait, callbacks, parent/child linking, a 16-thread cancel race and copy sharing. They also cover scope binding and restoring. The POSIX process tests use real subprocesses: a turn cancel, and a real SIGINT delivered to the main thread, both kill the backgrounded `sleep` grandchild during run_shell. A simulated Windows test uses a fake Popen and checks for the `CREATE_NEW_PROCESS_GROUP` flag and `taskkill /T /F`. Team tests show that a team cancel stops a member's in-flight run_shell and kills its child, that members get distinct child tokens, that the turn token stays live, and that no threads leak. Dispatch refuses after a cancel. Approval cancel is kept distinct in the prompt, dispatch outcome, REPL message and one-shot. The existing cancellation tests pass unchanged, apart from one, which now binds a team token instead of setting the removed attribute. These are local results on macOS only; no Windows console was exercised.

**Prevention:** New long-running or mutating work reads `cancellation.current_token()` and never creates a separate event. A pool thread started without `copy_context()` must bind its token explicitly.

## 2026-09-24: Ctrl+C At One Approval Prompt Labelled Every Later Batch Call As An Approval Cancel

**Symptom:** After Ctrl+C at one approval prompt in a sequential tool batch, every later call in that batch was recorded with `error_code=approval_cancelled` and the message "Approval was cancelled by the user; this action was not approved and was not run.". That included calls that needed no approval and were never prompted. Before the cancel-token change, those calls were recorded as `keyboard_interrupt` with "Action was cancelled before dispatch.".

**Confirmed cause:** `_DispatchControl.interrupt()` cancels the shared batch `DispatchCancellation` with the interrupt's own reason, which is `approval_cancelled`. The first reason wins, so the later `cancel("keyboard_interrupt")` in `main.dispatch_in_batch` did nothing. Each later call's `control.signal()` returned the batch reason unchanged, and `_termination_outcome` turned it into the approval message. The turn token path had the same flaw when the token itself was cancelled with `approval_cancelled`.

**Repair:** `_DispatchControl.signal()` maps a shared batch or token reason of `approval_cancelled` to `keyboard_interrupt` (`_follower_reason` in `algo_cli/james_dispatch.py`). The call whose own prompt was cancelled still reads its reason from its own interruption and keeps `approval_cancelled`. The batch cancellation still stores `approval_cancelled`, so `main` still raises `ApprovalCancelled` at the end of the batch and the REPL and one-shot messages are unchanged. No permission check changed; the follower calls are still refused before dispatch.

**Verification:** Two new tests in `tests/test_cancellation.py`. The first reproduces the report: a shared `DispatchCancellation`, an approval callback that raises `ApprovalCancelled` only on the first call, then a second call on the same batch. The second call is not prompted or invoked and now reports `keyboard_interrupt`, while the first keeps `approval_cancelled` and the batch still yields `ApprovalCancelled`. The second test covers a turn token cancelled with `approval_cancelled`. Both tests failed with the mapping removed and pass with it. Local macOS run only.

**Prevention:** A shared cancel signal carries why the batch stopped, not why each call stopped. Per-call labels that claim user intent, such as an approval decision, must come from that call's own event.
