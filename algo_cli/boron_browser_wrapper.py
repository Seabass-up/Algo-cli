...
class BoronNavigationPlan:
    ...
    def proxy_pac(self) -> bytes:
        """Return a deterministic PAC that exposes only the permitted origin."""

        target_host = urlsplit(self.canonical_url).hostname
        if target_host is None:
            _reject("navigation_url")
        host_literal = json.dumps(target_host, ensure_ascii=True)
        proxy_literal = json.dumps(
            f"PROXY {self.proxy_host}:{self.proxy_port}",
            ensure_ascii=True,
        )
        return (
            "function FindProxyForURL(url, host) {\n"
            f'  if (url.substring(0, 6) === "https:" && host === {host_literal}) {{\n'
            f"    return {proxy_literal};\n"
            "  }\n"
            '  return "DIRECT";\n'
            "}\n"
        ).encode("ascii")
