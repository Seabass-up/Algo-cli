"""One-shot non-interactive JSON event mode with bounded authority.
...
Event schema (one JSON object per line, no embedded raw newlines):
    ...
    {"type":"tool_result","call_id":...,"name":...,
                         "status":"ok|failed|denied|skipped|timed_out|cancelled|unknown_outcome",
        ...
        *,
        ...
        if outcome_status is None:
            ...
            if normalized not in _TYPED_TOOL_STATUSES:
                raise ValueError(f"unsupported typed tool outcome: {outcome_status!r}")
...
def run_oneshot(
    *,
    ...
    from .irene_privacy_views import volatile_privacy_key_scope
    ...
    if approval_mode not in {"never", "auto", "workspace"}:
        raise ValueError(
            "approval_mode must be 'never', 'auto', or 'workspace'"
        )
    ...
    try:
        ...
        for key, value in persistent_values.items():
            setattr(cfg, key, value)
        if hasattr(cfg, "_nathan_approval_mode"):
