...
LEGACY_AGENT_RUN_JOURNAL_SCHEMA_VERSION = 3
AGENT_RUN_ANCHOR_NAMESPACE = "agent-run-journal-v1"
AGENT_RUN_EVENT_AUTH_DOMAIN = "journal-event"
AGENT_RUN_ANCHOR_AUTH_DOMAIN = "journal-anchor"
AGENT_RUN_ANCHOR_ID_DOMAIN = b"algo-cli-agent-run-journal-anchor-v1"
MAX_ANCHOR_CAS_RETRIES = 4
ZERO_HASH = "0" * 64
...
_TERMINAL_BLOCK_STATUSES = frozenset({"complete", "partial", "failed", "cancelled"})
...
class AgentResumeState:
    ...
    terminal: bool
    terminal_status: str
    _receipt_key_store: Any | None = field(
        default=None,
        repr=False,
        compare=False,
    )
    ...
    def workspace_matches(self, snapshot: git_evidence.GitSnapshot) -> bool:
        expected = self.last_workspace or workspace_contract_view(self.contract.workspace)
        observed = workspace_view(
            snapshot,
            contract=self.contract,
            receipt_key_store=self._receipt_key_store,
        )
...
def workspace_view(
    ...
    *,
    contract: run_contract.RunContract | None = None,
    ...
    for field_name in (
        ...
        result[field_name] = contract.digest_workspace_state(
            result[field_name],
            field=field_name,
...
def _safe_identifier(value: Any, label: str, *, allow_empty: bool = False) -> str:
    ...
    if not _SAFE_ID_RE.fullmatch(text):
        raise AgentRunJournalError(f"{label} must be a bounded non-sensitive identifier")
    return text
...
def _next_block_ordinal(value: Any) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or not 0 <= value <= 32:
        raise AgentRunJournalError("next block ordinal must be an integer from 0 to 32")
    return value


def _round(value: Any) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or not 0 <= value < 2_048:
        raise AgentRunJournalError("model round must be an integer from 0 to 2047")
    return value


def _attempt(value: Any) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or not 0 <= value <= 1:
        raise AgentRunJournalError("attempt must be an integer from 0 to 1")
    return value
...
def _decode_event(
    ...
    *,
    ...
    hash_body: Callable[[Mapping[str, Any]], str],
) -> AgentRunEvent:
    ...
    if not hmac.compare_digest(hash_body(body), event_hash):
...
def _exact_payload(
    ...
    if set(payload) != expected:
        raise AgentRunJournalCorrupt(f"{kind} payload fields do not match schema")


def _validate_workspace_payload(raw: Any) -> None:
    if not isinstance(raw, dict):
        raise AgentRunJournalCorrupt("journal workspace payload must be an object")
    if type(raw.get("git_available")) is not bool:
        raise AgentRunJournalCorrupt("journal workspace availability must be boolean")
    ...
    if head and (len(head) not in {40, 64} or any(character not in "0123456789abcdef" for character in head)):
        raise AgentRunJournalCorrupt("journal workspace HEAD is invalid")
    for field_name in (
        "status_digest",
        "tracked_diff_digest",
        "untracked_digest",
    ...
    try:
        ...
        if kind == "run_resumed":
            ...
            if (
                ...
                raise AgentRunJournalError("resume sequence is outside the prior journal")
            return
        if kind == "context_bound":
            ...
            if context_schema_version not in {1, 2}:
                raise AgentRunJournalError("context receipt schema is unsupported")
            ...
            if (
                ...
                raise AgentRunJournalError("context token receipt is invalid")
            all_names: list[str] = []
            for field_name in (
                "included_sources",
                "truncated_sources",
                ...
                if not isinstance(values, list):
                    raise AgentRunJournalError("context sources must be lists")
                cleaned = [
                    _safe_identifier(
                        value,
                        "context source",
                ...
                if len(cleaned) != len(set(cleaned)):
                    raise AgentRunJournalError("context source list contains duplicates")
                if field_name != "truncated_sources":
                    all_names.extend(cleaned)
            if len(all_names) != len(set(all_names)):
                raise AgentRunJournalError("included and omitted context sources overlap")
            if not set(payload["truncated_sources"]).issubset(payload["included_sources"]):
                raise AgentRunJournalError("truncated context sources must be included")
            if context_schema_version == 2:
                source_metadata = payload["source_metadata"]
                if not isinstance(source_metadata, list):
                    raise AgentRunJournalError("context source metadata must be a list")
                ...
                for item in source_metadata:
                    ...
                    if not isinstance(item, dict) or set(item) != expected_metadata_fields:
                        raise AgentRunJournalError("context source metadata fields are invalid")
                    name = _safe_identifier(
                        item["name"],
                        "context metadata source",
                    )
                    if name in metadata_names:
                        raise AgentRunJournalError("context source metadata contains duplicates")
                    ...
                    if scope not in {"global", "workspace", "session"}:
                        raise AgentRunJournalError("context scope is invalid")
                    _safe_identifier(
                        item["trust"],
                        "context trust",
                    )
                    freshness = item["freshness_rank"]
                    if isinstance(freshness, bool) or not isinstance(freshness, int) or not 0 <= freshness <= 1_000_000:
                        raise AgentRunJournalError("context freshness rank is invalid")
                    _digest(
                        item["provenance_sha256"],
                        "context provenance digest",
                    )
                    if type(item["admitted"]) is not bool:
                        raise AgentRunJournalError("context admission flag is invalid")
                    ...
                    if item["admitted"]:
                        if reason:
                            raise AgentRunJournalError("admitted context has a rejection reason")
                        admitted_names.add(name)
                    else:
                        if not reason:
                            raise AgentRunJournalError("rejected context lacks a reason")
                        rejected_names.add(name)
                if admitted_names != set(payload["included_sources"]):
                    raise AgentRunJournalError("context admission metadata differs from included sources")
                if rejected_names != set(payload["omitted_sources"]):
                    raise AgentRunJournalError("context rejection metadata differs from omitted sources")
            return
        if kind == "block_started":
            ...
            if contract.block(ordinal).role != role:
                raise AgentRunJournalError("block role differs from the run contract")
            return
        if kind == "recovery_started":
            ...
            if attempt > block.max_recovery_attempts or code not in block.recovery_codes:
                raise AgentRunJournalError("recovery exceeds the run contract")
            return
        if kind == "recovery_finished":
            ...
            if attempt > contract.block(ordinal).max_recovery_attempts:
                raise AgentRunJournalError("recovery attempt exceeds the run contract")
            if payload["status"] not in _TERMINAL_BLOCK_STATUSES:
                raise AgentRunJournalError("recovery status is not terminal")
            _digest(payload["context_digest"], "context digest")
            return
        if kind in {"model_round_started", "model_round_completed"}:
            ...
            attempt = _attempt(payload["attempt"])
            if attempt > contract.block(ordinal).max_recovery_attempts:
                raise AgentRunJournalError("model attempt exceeds the run contract")
            if kind == "model_round_started":
                prompt_tokens = payload["prompt_tokens"]
                if (
                    isinstance(prompt_tokens, bool)
                    or not isinstance(prompt_tokens, int)
                    or not 0 <= prompt_tokens <= contract.budget.max_prompt_tokens_per_round
                ):
                    raise AgentRunJournalError("model prompt tokens exceed the run contract")
            if kind == "model_round_completed":
                _safe_identifier(
                    payload["status"],
                    "model round status",
                ...
                if isinstance(count, bool) or not isinstance(count, int) or not 0 <= count <= 256:
                    raise AgentRunJournalError("tool call count is invalid")
        ...
        if kind == "tool_intent":
            ...
            attempt = _attempt(payload["attempt"])
            if attempt > contract.block(ordinal).max_recovery_attempts:
                raise AgentRunJournalError("tool attempt exceeds the run contract")
            tool_index = payload["tool_index"]
            if isinstance(tool_index, bool) or not isinstance(tool_index, int) or not 0 <= tool_index < 256:
                raise AgentRunJournalError("tool index is invalid")
            expected_step = (
                f"b{ordinal}-r{round_number}-t{tool_index}"
                if attempt == 0
                else (f"b{ordinal}-a{attempt}-r{round_number}-t{tool_index}")
            )
            if payload["step_id"] != expected_step:
                raise AgentRunJournalError("tool step ID does not match its coordinates")
            ...
            if type(payload["mutating"]) is not bool:
                raise AgentRunJournalError("tool mutation flag must be boolean")
        ...
        if kind == "tool_result":
            ...
            if type(payload["invoked"]) is not bool or type(payload["deduplicated"]) is not bool:
                raise AgentRunJournalError("tool result flags must be boolean")
            return
        if kind == "verifier_result":
            ...
            if payload["status"] not in {"passed", "failed"}:
                raise AgentRunJournalError("verifier status must be passed or failed")
        ...
        if kind == "block_finished":
            ...
            if payload["role"] != contract.block(ordinal).role:
                raise AgentRunJournalError("finished block role differs from the run contract")
            if payload["status"] not in _TERMINAL_BLOCK_STATUSES:
                raise AgentRunJournalError("finished block status is not terminal")
            if type(payload["verified"]) is not bool:
                raise AgentRunJournalError("finished block verification flag is invalid")
            _digest(payload["context_digest"], "context digest")
            _validate_workspace_payload(payload["workspace"])
            return
        if kind == "run_finished":
            ...
            if (
                ...
                raise AgentRunJournalError("last verified sequence is invalid")
            return
        ...
        raise AgentRunJournalCorrupt(f"{kind} payload is invalid: {exc}") from exc
...
def _validate_event_sequence(
    ...
    def reject(reason: str) -> NoReturn:
        raise AgentRunJournalCorrupt(f"journal event sequence is invalid: {reason}")
    ...
    def batch_is_closed() -> bool:
        ...
        return len(intents) == open_batch["expected"] and results == set(intents)
    ...
    for index, event in enumerate(events):
        ...
        if kind == "block_finished":
            ...
            if verified:
                ...
                if any(verifier_status.get(verifier) != "passed" for verifier in required):
                    reject("verified block lacks its passed verifiers")
                workspace = payload["workspace"]
                if any(verifier_workspace.get(verifier) != workspace for verifier in required):
                    reject("verified block workspace differs from verifier evidence")
                if not checkpoint_gap and ordinal == len(completed_ordinals):
...
class AgentRunJournal:
    ...
    def __init__(
        ...
        *,
        ...
        protected_expected: bool | None = None,
    ) -> None:
        if not isinstance(contract, run_contract.RunContract):
            raise AgentRunJournalError("journal requires a RunContract")
        if protected_expected is not None and type(protected_expected) is not bool:
            raise AgentRunJournalError("protected journal expectation must be boolean")
        if protected_expected is True and contract.schema_version == run_contract.LEGACY_RUN_CONTRACT_SCHEMA_VERSION:
            raise AgentRunJournalError(
                "legacy Agent journal has no protected digest binding; "
                "migration is unsafe, so start a new protected run"
            )
        if protected_expected is not None and contract.protected_digests != protected_expected:
            raise AgentRunJournalError("Agent journal digest authority differs from the active memory authority")
        try:
            contract.assert_sensitive_digest_key(key_store=receipt_key_store)
        except run_contract.RunContractError as exc:
            raise AgentRunJournalError(str(exc)) from exc
        anchor_store = receipt_anchor_store
        if contract.protected_digests:
            if (
                anchor_store is None
                and receipt_key_store is not None
                and all(callable(getattr(receipt_key_store, method, None)) for method in ("load", "compare_and_set"))
            ):
                anchor_store = receipt_key_store
            elif anchor_store is None and receipt_key_store is None:
                anchor_store = grace_key_store.GraceReceiptAnchorStore()
            if anchor_store is None or any(
                not callable(getattr(anchor_store, method, None)) for method in ("load", "compare_and_set")
            ):
                raise AgentRunJournalError("protected Agent journal receipt anchor is unavailable")
        elif anchor_store is not None:
            raise AgentRunJournalError("unprotected Agent journals cannot claim a receipt anchor")
        self.contract = contract
        self.store = store
        self._receipt_key_store = receipt_key_store
        self._receipt_anchor_store: Any = anchor_store
        self._anchor_pending = False
        self._anchor_bootstrap_allowed = False

    @property
    def protected(self) -> bool:
        return self.contract.protected_digests

    @property
    def schema_version(self) -> int:
        return (
            LEGACY_AGENT_RUN_JOURNAL_SCHEMA_VERSION
            if self.contract.schema_version == run_contract.LEGACY_RUN_CONTRACT_SCHEMA_VERSION
            else AGENT_RUN_JOURNAL_SCHEMA_VERSION
        )

    @property
    def anchor_id(self) -> str:
        encoded = (
            AGENT_RUN_ANCHOR_ID_DOMAIN
            + b"\0"
            + self.contract.run_nonce.encode("utf-8")
            + b"\0"
            + self.contract.digest.encode("ascii")
        )
        return "sha256:" + hashlib.sha256(encoded).hexdigest()

    def _event_hash(self, body: Mapping[str, Any]) -> str:
        try:
            return self.contract.digest_sensitive_json(
                dict(body),
                domain=AGENT_RUN_EVENT_AUTH_DOMAIN,
                key_store=self._receipt_key_store,
            )
        except run_contract.RunContractError as exc:
            raise AgentRunJournalError(str(exc)) from exc

    def _anchor_head(self, event: AgentRunEvent) -> grace_key_store.ContentFreeReceiptHead:
        unsigned = {
            "kind": grace_key_store.ContentFreeReceiptHead.KIND,
            "schema_version": 1,
            "namespace": AGENT_RUN_ANCHOR_NAMESPACE,
            "journal_id": self.anchor_id,
            "subject_digest": self.contract.digest,
            "sequence": event.sequence,
            "head_digest": event.event_hash,
        }
        try:
            authentication = self.contract.digest_sensitive_json(
                unsigned,
                domain=AGENT_RUN_ANCHOR_AUTH_DOMAIN,
                key_store=self._receipt_key_store,
            )
            return grace_key_store.ContentFreeReceiptHead(
                namespace=AGENT_RUN_ANCHOR_NAMESPACE,
                journal_id=self.anchor_id,
                subject_digest=self.contract.digest,
                sequence=event.sequence,
            ...
            grace_key_store.ReceiptAnchorStoreError,
            run_contract.RunContractError,
        ) as exc:
            raise AgentRunJournalError("Agent journal receipt anchor is invalid") from exc

    @staticmethod
    def _anchor_blob_digest(value: bytes) -> str:
        return "sha256:" + hashlib.sha256(value).hexdigest()

    def _load_anchor(
        self,
    ) -> tuple[grace_key_store.ContentFreeReceiptHead, bytes] | None:
        if not self.protected:
            return None
        try:
            blob = self._receipt_anchor_store.load(self.anchor_id)
        except Exception:
            raise AgentRunJournalError("Agent journal receipt anchor is unavailable") from None
        if blob is None:
            return None
        try:
            head = grace_key_store.ContentFreeReceiptHead.from_bytes(blob)
        except grace_key_store.ReceiptAnchorStoreError as exc:
            raise AgentRunJournalCorrupt("Agent journal receipt anchor is malformed") from exc
        if (
            head.namespace != AGENT_RUN_ANCHOR_NAMESPACE
            or head.journal_id != self.anchor_id
            or head.subject_digest != self.contract.digest
        ):
            raise AgentRunJournalCorrupt("Agent journal receipt anchor belongs to different authority")
        try:
            expected_authentication = self.contract.digest_sensitive_json(
                head.unsigned_payload(),
                domain=AGENT_RUN_ANCHOR_AUTH_DOMAIN,
                key_store=self._receipt_key_store,
            )
        except run_contract.RunContractError as exc:
        ...
        if not hmac.compare_digest(
            ...
            raise AgentRunJournalCorrupt("Agent journal receipt anchor authentication failed")
        return head, blob

    def _anchor_state(
        self,
        records: tuple[AgentRunEvent, ...],
    ) -> tuple[bool, bytes | None]:
        """Return whether the durable journal is exactly one event ahead."""
        ...
        if not records:
            if loaded is not None:
                raise AgentRunJournalCorrupt("Agent journal receipt head is ahead of the journal")
            return False, None
        if loaded is None:
            if len(records) == 1 and self._anchor_bootstrap_allowed:
                return True, None
            raise AgentRunJournalCorrupt("Agent journal receipt anchor is missing")
        head, blob = loaded
        if head.sequence >= len(records):
            raise AgentRunJournalCorrupt("Agent journal receipt head rollback detected")
        anchored = records[head.sequence]
        if not hmac.compare_digest(anchored.event_hash, head.head_digest):
            raise AgentRunJournalCorrupt("Agent journal receipt head differs from the chain")
        if head.sequence == len(records) - 1:
            return False, blob
        if head.sequence == len(records) - 2:
            return True, blob
        raise AgentRunJournalCorrupt("Agent journal receipt head rollback detected")

    def _synchronize_anchor(
        self,
        records: tuple[AgentRunEvent, ...],
    ) -> None:
        if not self.protected or not records:
            return
        for _attempt in range(MAX_ANCHOR_CAS_RETRIES):
            pending, prior_blob = self._anchor_state(records)
            if not pending:
                self._anchor_pending = False
                return
            candidate = self._anchor_head(records[-1]).to_bytes()
            expected_digest = None if prior_blob is None else self._anchor_blob_digest(prior_blob)
            try:
                changed = self._receipt_anchor_store.compare_and_set(
                    self.anchor_id,
            ...
            if type(changed) is not bool:
                raise AgentRunJournalError("Agent journal receipt anchor is unavailable")
            if changed:
                self._anchor_pending = False
                return
        raise AgentRunJournalError("Agent journal receipt anchor update raced")

    def synchronize_anchor(self) -> None:
        """Explicitly reconcile the one-event append/anchor crash window."""

        append_target = self.store.path.with_suffix(self.store.path.suffix + ".append")
        try:
            with config._exclusive_state_lock(append_target):
                records = self.records()
                self._synchronize_anchor(records)
        except TimeoutError as exc:
            raise AgentRunJournalError("another process is updating this Agent journal") from exc

    def _persist_digest(self, value: Any, *, domain: str) -> str:
        raw_digest = _digest(value, f"{domain} digest")
        if not self.protected:
            return raw_digest
        try:
            return self.contract.digest_sensitive_text(
                raw_digest,
                domain=domain,
                key_store=self._receipt_key_store,
            )
    ...
    def digest_text(self, value: str, *, domain: str) -> str:
        try:
            return self.contract.digest_sensitive_text(
                value,
                domain=domain,
    ...
    def digest_json(self, value: Any, *, domain: str) -> str:
        try:
            return self.contract.digest_sensitive_json(
                value,
                domain=domain,
    ...
    def checkpoint_digest_text(self, value: str, *, domain: str) -> str:
        """Reproduce a stored receipt derived from a caller SHA-256 digest."""

        return self._persist_digest(
            digest_text(value),
            domain=domain,
        )
    ...
    def create(
        ...
        *,
        ...
        journal._anchor_bootstrap_allowed = journal.protected
        try:
            journal._append(
                "run_started",
            ...
            journal._anchor_bootstrap_allowed = False
        return journal
    ...
    def load(
        ...
        *,
        ...
        try:
            ...
            raise AgentRunJournalCorrupt("persisted run contract is invalid") from exc
    ...
    def records(self) -> tuple[AgentRunEvent, ...]:
        ...
        for sequence, raw in enumerate(self.store.read_events()):
            event = _decode_event(
                ...
                hash_body=self._event_hash,
            )
            if sequence == 0:
                if event.kind != "run_started":
                    raise AgentRunJournalCorrupt("journal must begin with run_started")
                try:
                    persisted = run_contract.RunContract.from_payload(event.payload.get("contract"))
                except run_contract.RunContractError as exc:
                    raise AgentRunJournalCorrupt("journal start contract is invalid") from exc
                if persisted.digest != self.contract.digest:
                    raise AgentRunJournalCorrupt("journal start contract digest differs")
            elif event.kind == "run_started":
                raise AgentRunJournalCorrupt("run_started may appear only once")
            _validate_event_payload(event, self.contract)
            decoded.append(event)
            previous_hash = event.event_hash
        result = tuple(decoded)
        _validate_event_sequence(result, self.contract)
        pending, _blob = self._anchor_state(result)
        self._anchor_pending = pending
        return result
    ...
    def execution_lease(
        ...
        *,
        ...
        lease_target = self.store.path.with_suffix(self.store.path.suffix + ".execution")
        try:
            with config._exclusive_state_lock(
                lease_target,
                timeout_seconds=timeout_seconds,
            ):
                self.synchronize_anchor()
                yield
        except TimeoutError as exc:
            raise AgentRunJournalError("another process is already executing this Agent run") from exc
    ...
    def _append(self, kind: str, payload: Mapping[str, Any]) -> AgentRunEvent:
        ...
        try:
            with config._exclusive_state_lock(append_target):
                ...
                next_records = (*records, candidate)
                _validate_event_sequence(next_records, self.contract)
                result = self.store.append(encoded)
                if not result.stored:
                    raise AgentRunJournalError("journal event was not durably stored")
                self._synchronize_anchor(next_records)
                return candidate
        except TimeoutError as exc:
            raise AgentRunJournalError("another process is updating this Agent journal") from exc
    ...
    def run_resumed(
        ...
        *,
        ...
        if (
            ...
            raise AgentRunJournalError("last_verified_sequence must be at least -1")
        return self._append(
            "run_resumed",
            {
                "next_block_ordinal": _next_block_ordinal(next_block_ordinal),
    ...
    def context_bound(
        ...
        if not isinstance(receipt, Mapping):
            raise AgentRunJournalError("Agent context receipt fields do not match schema")
        ...
        payload = {
            ...
            "context_digest": self._persist_digest(
                receipt["context_digest"],
                domain="bound-context",
            ),
            "max_tokens": receipt["max_tokens"],
            "base_tokens": receipt["base_tokens"],
            "used_tokens": receipt["used_tokens"],
            "included_sources": list(receipt["included_sources"]),
            "truncated_sources": list(receipt["truncated_sources"]),
            "omitted_sources": list(receipt["omitted_sources"]),
        }
        if schema_version == 2:
            source_metadata: list[dict[str, Any]] = []
            for raw_item in receipt["source_metadata"]:
                item = dict(raw_item)
                item["provenance_sha256"] = self._persist_digest(
                    item.get("provenance_sha256"),
                    domain="context-provenance",
                )
    ...
    def model_round_started(
        ...
        *,
        ...
        if isinstance(prompt_tokens, bool) or not isinstance(prompt_tokens, int) or prompt_tokens < 0:
            raise AgentRunJournalError("prompt tokens must be a nonnegative integer")
        return self._append(
            "model_round_started",
            {
                "ordinal": _ordinal(ordinal, "block ordinal"),
    ...
    def model_round_completed(
        ...
        *,
        ...
        if isinstance(tool_call_count, bool) or not isinstance(tool_call_count, int) or not 0 <= tool_call_count <= 256:
            raise AgentRunJournalError("tool_call_count must be an integer from 0 to 256")
        return self._append(
            "model_round_completed",
            {
                "ordinal": _ordinal(ordinal, "block ordinal"),
                ...
                "response_digest": self._persist_digest(
                    response_digest,
    ...
    def tool_intent(
        ...
        *,
        ...
        if isinstance(tool_index, bool) or not isinstance(tool_index, int) or not 0 <= tool_index < 256:
            raise AgentRunJournalError("tool index must be an integer from 0 to 255")
        ...
        step_id = (
            f"b{ordinal}-r{round_number}-t{tool_index}"
            if clean_attempt == 0
            else (f"b{ordinal}-a{clean_attempt}-r{round_number}-t{tool_index}")
        )
        return self._append(
            "tool_intent",
            {
                ...
                "args_digest": self.digest_json(
                    dict(args),
                    domain="tool-arguments",
                ),
                "call_id_hash": self.digest_text(
                    call_id,
                    domain="tool-call-id",
                ),
                "mutating": mutating,
    ...
    def tool_result(
        ...
        *,
        ...
        if type(invoked) is not bool or type(deduplicated) is not bool:
            raise AgentRunJournalError("tool result booleans are invalid")
        return self._append(
            "tool_result",
            {
                ...
                "idempotency_key": (
                    self._persist_digest(
                        idempotency_key,
                        domain="tool-idempotency-key",
                    )
        ...
        *,
        ...
        return self._append(
            "verifier_result",
            {
                ...
                "workspace": self._workspace_view(snapshot),
            },
        )
    ...
    def resume_state(self) -> AgentResumeState:
        ...
        for event in self.records():
            ...
            if event.kind == "tool_intent":
                ...
                if step_id in pending_intents:
                    raise AgentRunJournalCorrupt("tool intent step ID is duplicated")
                ...
                step_id = str(payload.get("step_id") or "")
                if step_id not in pending_intents:
                    raise AgentRunJournalCorrupt("tool result has no matching intent")
                pending_intents.pop(step_id)
                ...
                ordinal = payload.get("ordinal")
                if payload.get("status") == "complete" and payload.get("verified") is True:
                    ...
                    if ordinal == expected and not checkpoint_gap:
                        ...
                        raise AgentRunJournalCorrupt("verified block checkpoint is duplicated or out of order")
                    else:
                        checkpoint_gap = True
        ...
        uncertain_steps = {step_id for step_id, payload in pending_intents.items() if payload.get("mutating") is True}
        ...
        return AgentResumeState(
            ...
            _receipt_key_store=self._receipt_key_store,
        )
    ...
    def verified_blocks(self) -> tuple[VerifiedBlockCheckpoint, ...]:
        ...
        for event in self.records():
            ...
            if (
                ...
                or payload.get("ordinal") not in state.completed_block_ordinals
            ):
                continue
            workspace = payload.get("workspace")
            checkpoints.append(
        ...
        if tuple(item.ordinal for item in checkpoints) != (state.completed_block_ordinals):
            raise AgentRunJournalCorrupt("verified block checkpoints differ from resume state")
    ...
    def checkpoint_payload(self) -> dict[str, Any]:
        ...
        return {
            ...
            "uncertain_mutation_steps": list(state.uncertain_mutation_steps),
