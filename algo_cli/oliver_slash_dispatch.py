"""Oliver slash-command table, completion, and REPL dispatch."""
...
def handle_command(raw: str, cfg: Config, client: Client, session: Any = None) -> tuple[bool, Client]:
    ...
    if command == "/help":
        ...
        try:
            subcommand = (arg.strip().split(maxsplit=1) or ["home"])[0].lower()
            embed_fn = None
            if subcommand in {"search", "benchmark", "reindex"}:
                embed_fn = m.intuition_embed_fn(cfg)
            memory_text = memory_runtime.command_text(
                arg,
                cfg,
                embed_fn=embed_fn,
        ...
        if not arg:
            ...
            try:
                added = memory_runtime.remember_fact(cfg, arg)
            except memory_runtime.MemorySystemError as exc:
                m.show_error(str(exc))
            else:
                if added:
        ...
        try:
            ...
            m.show_error(f"Memory deletion failed: {exc}")
    elif command == "/clear":
        cfg.messages.clear()
        cfg.session_summary = ""
        ...
        if sub in ("list", ""):
            ...
            if not statuses:
                m.show_info("No plugin manifests found.")
            else:
                from rich.table import Table as _Table
                from rich.text import Text as _Text
                ...
                for s in statuses:
                    table.add_row(
                        _Text(str(s.get("name", "?"))),
                        _Text(str(s.get("state", "unknown"))),
                        "yes" if s.get("loaded") else "no",
                        _Text(str(s.get("load_error", "") or "")),
                    )
                m.console.print(table)
        else:
