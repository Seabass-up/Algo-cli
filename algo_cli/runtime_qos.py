"""Runtime QoS and named log helpers (J6 + J7).
...
_LOW_COST_TOOL_NAMES = frozenset({
    "read_file",
    "list_directory",
    "search_files",
    "find_unique_anchor",
...
def classify_tool_runtime(tool_name: str, args: dict[str, Any] | None = None) -> RuntimeHint:
    """Classify a tool call into a launchd-style runtime posture."""
    name = (tool_name or "").strip()
    args = args or {}
    suppress = name in SENSITIVE_TOOL_NAMES or bool(args.get("suppress_log"))
    if name in BACKGROUND_TOOL_NAMES:
        spawn = SpawnClass.BACKGROUND
        reason = "known long-running/background maintenance tool"
        estimated_cost = 8.0
    elif name in INTERACTIVE_TOOL_NAMES:
        spawn = SpawnClass.INTERACTIVE
        reason = "session/user-facing command"
        estimated_cost = 1.0
    elif name == "run_shell":
        command = str(args.get("command", "")).lower()
        if any(token in command for token in ("pytest", "ruff", "mypy", "npm test", "cargo test")):
            spawn = SpawnClass.ADAPTIVE
            reason = "verification command; adaptive runtime"
            estimated_cost = 4.0
        else:
            spawn = SpawnClass.INTERACTIVE
            reason = "shell command may require user-facing output"
            estimated_cost = 2.0
    elif name in _LOW_COST_TOOL_NAMES:
        spawn = SpawnClass.ADAPTIVE
        reason = "bounded read-only operation"
        estimated_cost = 1.0
    else:
        spawn = SpawnClass.BACKGROUND if suppress else SpawnClass.ADAPTIVE
        reason = "default runtime classification"
        estimated_cost = 2.0
