...
def local_service_address(url: str, *, require_http: bool = False) -> str | None:
    """Return a credential-free loopback host:port for a local service URL."""
    ...
    try:
        ...
        return None
    if parsed.scheme not in ({"http"} if require_http else {"http", "https"}):
        return None
    if (
        not parsed.hostname
...
def gateway_ready(url: str | None = None) -> bool:
    ...
    if local_service_address(url, require_http=True) is None:
        return False
...
def start_supplemental_gateway(cfg: Config) -> bool:
    ...
    if not start_local_ollama_host(cfg.host):
        return False
    url = gateway_url()
    addr = local_service_address(url, require_http=True)
    if addr is None:
        show_error("Supplemental gateway URL must be an explicit credential-free loopback HTTP endpoint.")
        return False
