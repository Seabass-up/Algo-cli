"""Julia contracts for the curated, category-driven Algo CLI product memory set."""
...
def test_curated_project_source_tuples_are_contract_focused() -> None:
    ...
    runtime_docs_root = harness._algo_cli_docs_dir()
...
def test_reviewed_wiki_docs_expose_runtime_and_evidence_boundaries() -> None:
    external = (DOCS_ROOT / "external-agent-store-operations.md").read_text(
        encoding="utf-8"
    )
    privacy = (DOCS_ROOT / "privacy-and-context.md").read_text(encoding="utf-8")
    graph = (DOCS_ROOT / "index-compute-lab-integration.md").read_text(
        encoding="utf-8"
    )
    ...
    assert privacy_frontmatter["updated"] == "2026-07-24"
    assert privacy_frontmatter["last_reviewed"] == "2026-07-24"
