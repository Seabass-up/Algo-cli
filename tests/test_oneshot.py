...
from algo_cli import display, oneshot
...
def test_oneshot_ask_approval_allows_dangerous_under_auto(monkeypatch):
    from algo_cli import main as main_module
    ...
    oneshot.run_oneshot(prompt="x", approval_mode="auto", stream=io.StringIO())
    assert decisions == [("read_file", True), ("write_file", True), ("run_shell", True)]
