...
def test_echo_dependency_is_commit_pinned_and_exercised_in_ci() -> None:
    ...
    assert (
        "uv run --frozen --no-editable --extra dev --extra supply-chain --group echo-veil --link-mode copy pytest tests"
    ) in workflow
    assert (
        "uv sync --frozen --no-editable --extra dev --group echo-veil "
        "--reinstall-package algo-cli-runtime --link-mode copy"
    ) in workflow
    assert ("uv run --frozen --no-editable --extra dev --group echo-veil --link-mode copy pytest tests") in workflow
...
@pytest.mark.skipif(os.name == "nt", reason="Windows symlink creation requires host privileges")
def test_verify_distribution_record_accepts_install_prefix_alias(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    from algo_cli import memory_echo_veil

    install_prefix = tmp_path / "physical" / "venv"
    install_prefix.mkdir(parents=True)
    install_alias = tmp_path / "venv-alias"
    install_alias.symlink_to(install_prefix, target_is_directory=True)
    monkeypatch.setattr(memory_echo_veil.sys, "prefix", str(install_alias))

    py_size = 17
    py_digest = (
        base64.urlsafe_b64encode(hashlib.sha256(b"x" * py_size).digest())
        .rstrip(b"=")
        .decode("ascii")
    )
    dist = _fake_distribution_with_files(
...
@pytest.mark.skipif(os.name == "nt", reason="Windows symlink creation requires host privileges")
def test_verify_distribution_record_rejects_symlinked_member(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    from algo_cli import memory_echo_veil
    ...
    monkeypatch.setattr(memory_echo_veil.sys, "prefix", str(install_prefix))
    ...
    dist = _fake_distribution_with_files(
        ...
        [
            ("echo_veil/agent_memory.py", py_digest, py_size),
            ("echo_veil/agent_memory_alias.py", py_digest, py_size),
        ],
    )
    alias = install_prefix / "echo_veil" / "agent_memory_alias.py"
    alias.unlink()
    alias.symlink_to("agent_memory.py")

    with pytest.raises(memory_echo_veil._EchoModuleIdentityError, match="installed_file_symlink"):
        memory_echo_veil._verify_distribution_record(dist)
...
@pytest.mark.parametrize(
    "untrusted_name",
    [
        "echo_veil/schema.json",
        "echo_veil/__pycache__/payload.bin",
        "echo_veil/agent_memory.pyc",
    ],
)
def test_verify_distribution_record_rejects_other_hashless_files(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    untrusted_name: str,
) -> None:
    ...
    # Only generated bytecode caches qualify for the narrow pip RECORD exception.
    dist = _fake_distribution_with_files(
        tmp_path,
        [
            ("echo_veil/agent_memory.py", py_digest, py_size),
            (untrusted_name, None, 2),
        ],
    )
